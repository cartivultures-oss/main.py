...
# We now store the interaction itself to update the message in-channel
active_interactions = {}
...
async def run_bump(user_id, slot, config):
    ...
    async with async_playwright() as p:
        ...
        context = await browser.new_context(user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/110.0.0.0 Safari/537.36")
        ...
        try:
            ...
            success = "posted" in page.url or tid in page.url
            await browser.close()
            return "✅ Success" if success else "❌ Forum Rejected"
...
async def update_status_msg(interaction, slot, config):
    try:
        ...
        embed = discord.Embed(title=f"OGUser Bumper - Slot {slot}", color=0x2ecc71 if "✅" in config.get('last_status', '') else 0x3498db)
        ...
        embed.add_field(name="Last Status", value=config.get('last_status', 'Working...'), inline=False)
        embed.set_footer(text=f"Last Sync: {datetime.now().strftime('%H:%M:%S')}")
        
        # This updates the original private reply in the channel
...
async def start(ctx, slot: int):
    ...
    config['active'] = True
    active_interactions[key] = ctx.interaction # Save the interaction for updates
    
    await ctx.send(f"🚀 **Launching Browser for Slot {slot}...**", ephemeral=True)
